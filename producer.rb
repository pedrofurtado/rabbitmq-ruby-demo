...
puts "[P] Creating channel and queue, if not exists ..."
...
puts "[P] Channel created! Queue '#{queue_name}' also created!"
...
while true
  ...
  channel.default_exchange.publish(message, routing_key: queue_name, persistent: durable)
  puts "[P] Message sent to queue '#{queue.name}': '#{message}'"
end
