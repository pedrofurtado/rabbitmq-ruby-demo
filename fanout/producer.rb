...
exchange = channel.fanout('fila_do_produtor_fanout', durable: durable)
exchange_name = exchange.name
puts "[P] Channel created! Fanout exchange '#{exchange_name}' also created!"
...
while true
  ...
  exchange.publish(message, routing_key: exchange_name, persistent: durable)
  puts "[P] Message sent to exchange '#{exchange.name}': '#{message}'"
end
